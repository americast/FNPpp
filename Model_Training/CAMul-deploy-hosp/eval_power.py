...
writer = SummaryWriter("runs/eval/power_"+slide_text+"_"+str(options.file_names))
...
#                     ignore_index=True,
#                 )
with open("val_predictions_power"+"/"+options.file_names+".pkl", "rb") as f:
...
for ep in tqdm(data_dict.keys()):
    ...
    pred_here = np.array(pred)
    gt_here = np.array(gt)
    vars_here = np.array(vars)
