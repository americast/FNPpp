    ...
    for segment in segments:
        for adapt in adaptive:
            for ah in ahead:
                ...
                std = np.array(std).T
                plot_round(gt, mean, std, f"ar_seg_stable_{sample}_{segment}_{adapt}/{ah}")
...
metrics_all_weeks = metrics_df.groupby(["sample_out", "ahead", "segments", "adaptive"]).mean().reset_index()
