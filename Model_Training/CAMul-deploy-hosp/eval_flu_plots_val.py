        ...
        for ah in week_ahead:
            if "slidingwindow" in model_type or "preprocess" in model_type:
                if "autosize" in model_type:
                    save_model = f"slidingwindow_disease_flu_epiweek_"+str(epiweek)+"_weekahead_"+str(ah)+"_autosize_"+str(model_type[-1])
                else:
                    save_model = f"slidingwindow_disease_flu_epiweek_"+str(epiweek)+"_weekahead_"+str(ah)+"_wsize_"+str(sizes[mt])+"_wstride_"+str(strides[mt])

                if "preprocess" in model_type:
                    save_model = f"slidingwindowpreprocessed_disease_flu_epiweek_"+str(epiweek)+"_weekahead_"+str(ah)+"_wsize_"+str(sizes[mt])+"_wstride_"+str(strides[mt])
                ...
                save_model = "disease_flu_epiweek_"+str(epiweek)+"_weekahead_"+str(ah)
                if "cnn" in model_type:
                    save_model = "cnn_"+save_model
                elif "rag" in model_type:
                    save_model = "rag_"+save_model
