    ...
    for ah in week_ahead:
        if "slidingwindow" in model_type or "preprocess" in model_type:
            if "autosize" in model_type:
                save_model = f"slidingwindow_disease_symp_weekahead_"+str(ah)+"_autosize_"+str(model_type[-1])
            else:
                try:
                    save_model = f"slidingwindow_disease_symp_weekahead_"+str(ah)+"_wsize_"+str(sizes[mt])+"_wstride_"+str(strides[mt])
                except: pu.db

            if "preprocess" in model_type:
                save_model = f"slidingwindowpreprocessed_disease_symp_weekahead_"+str(ah)+"_wsize_"+str(sizes[mt])+"_wstride_"+str(strides[mt])
        ...
        #     plt.colorbar()
        #     plt.savefig("symp_plot_"+str(a)+".png")
        #     plt.close()
        # pu.db
        # yp_this_week
