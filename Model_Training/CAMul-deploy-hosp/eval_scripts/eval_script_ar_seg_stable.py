...
def get_metrics(file_name, epiweek_now, ahead):
    ...
    predictions = fl_data[0][None, :, ahead - 1]
    ground_truth = fl_data[1][None, ahead - 1]
...
def plot_round(ground_truth, mean_preds, std_preds, title, state):
    for i in range(ground_truth.shape[0]):
        ...
        plt.savefig(f"plots_seg_stable/{title}/{state}.png")
            ...
            for ah in ahead:
                for state in states:
                    ...
                    plot_round(
                        ...
                        mean,
                        std,
                        f"ar_seg_stable_{sample}_{segment}_{adapt}/{ah}",
                        state,
                    )
