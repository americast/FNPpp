...
with open("mormal_metrics_stable.pkl", "wb") as f:
    pickle.dump(metrics_df, f)

# Save as csv
metrics_df.to_csv("normal_metrics_stable.csv", index=False)

# Avergae over all epiweeks
metrics_all_weeks = metrics_df.groupby(["sample_out", "lr", "ahead"]).mean().reset_index()
metrics_all_weeks.to_csv("normal_metrics_stable_all_weeks.csv", index=False)
