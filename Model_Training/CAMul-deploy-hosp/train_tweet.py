...
parser.add_option("-e", "--epoch", dest="epochs", type="int", default="100")
...
full_sequences = np.array([np.array(
    [get_week(w, r) for w in np.arange(start_week, pred_week + 1)]) for r in states])
...
X, X_smart, X_symp, X_symp_smart, Y, mt, reg, static = [], [], [], [], [], [], [], []
for i, s in enumerate(states):
    ans = seq_to_dataset(full_sequences[i, :train_weeks[-1]], state_idx[s])
    ans_smart = seq_to_dataset(smoothen(full_sequences[i, :train_weeks[-1]]), state_idx[s])
    X.extend([x[:, topic] for x in ans[0]])
    X_symp.extend([x for x in ans[0]])
    X_smart.extend([x[:, topic] for x in ans_smart[0]])
    X_symp_smart.extend([x for x in ans_smart[0]])
    Y.extend([x[topic] for x in ans[1]])
    mt.extend(ans[2])
    reg.extend(ans[3])
    static.extend(ans[4])

X_test, X_symp_test, X_test_smart, X_symp_test_smart, Y_test, mt_test, reg_test, static_test = [], [], [], [], [], [], [], []
for i, s in enumerate(states):
    ans = seq_to_dataset(full_sequences[i], state_idx[s], pred_week)
    ans_smart = seq_to_dataset(smoothen(full_sequences[i]), state_idx[s], pred_week)
    X_test.append(full_sequences[i, :train_weeks[-1], topic])
    X_symp_test.append(full_sequences[i, :train_weeks[-1]])
    X_test_smart.append(full_sequences[i, :train_weeks[-1], topic])
    X_symp_test_smart.append(full_sequences[i, :train_weeks[-1]])
...
stoch_month_enc = LatentEncoder(in_dim=60, hidden_layers=[
                                60], out_dim=60).to(device)
stoch_seq_enc = LatentEncoder(in_dim=60, hidden_layers=[
                              60], out_dim=60).to(device)
stoch_symp_enc = LatentEncoder(in_dim=60, hidden_layers=[
                               60], out_dim=60).to(device)
stoch_reg_enc = LatentEncoder(in_dim=60, hidden_layers=[
                              60], out_dim=60).to(device)
stoch_static_enc = LatentEncoder(in_dim=60, hidden_layers=[
                                 60], out_dim=60).to(device)

if "fft" in options.optionals:
    seq_encoder_fft = GRUEncoder(in_size=1, out_dim=60).to(device)
    symp_encoder_fft = GRUEncoder(in_size=30, out_dim=60).to(device)
    stoch_seq_enc_fft = LatentEncoder(in_dim=60, hidden_layers=[
                              60], out_dim=60).to(device)
    stoch_symp_enc_fft = LatentEncoder(in_dim=60, hidden_layers=[
                               60], out_dim=60).to(device)
...
decoder = Decoder(z_dim=60, sr_dim=60, latent_dim=60,
                  hidden_dim=60, y_dim=1).to(device)
...
if "combine" in options.optionals:
    ...
    combine_symp = Combine(30).to(device)
    models += [
        combine_seq,
        combine_symp,
    ]
...
def train(train_seqs, train_seqs_smart, train_symp_seqs, train_symp_seqs_smart, reg, mt, train_y):
    ...
    ref_reg = reg_encoder.forward(
        long_tensor(reg_references), graph.to(device))
...
f = open("tweet_results/ahead_"+str(options.week_ahead)+"_optionals_"+str(options.optionals)+"_seed_"+str(options.seed), "w")
